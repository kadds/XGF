...
namespace XGF
{
	...
	ID3D11Buffer* IConstantBuffer::GetRawBuffer()
	{
		return mRawBuffer;
	}

	void IConstantBuffer::SetName(char* name)
	{
		this->name = name;
	}

	char* IConstantBuffer::GetName()
	{
		return name;
	}
	...
	ConstantBuffer::ConstantBuffer(): buffer(nullptr), size(0)
	{
	}

	void ConstantBuffer::Initialize(UINT len)
	{
		auto& gdi = Context::Current().QueryGraphicsDeviceInterface();

		mRawBuffer = CreateBuffer(gdi, D3D11_BIND_CONSTANT_BUFFER, 0, len, 0, D3D11_USAGE_DYNAMIC,
	...
	int ConstantBuffer::GetSize()
	{
		return size;
	}

	void* ConstantBuffer::GetBufferPoint()
	{
		return buffer;
	}
