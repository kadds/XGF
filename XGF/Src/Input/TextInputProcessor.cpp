...
namespace XGF
{
	...
	void TextInputProcessor::RenderText(SM::Color & color)
	{
		...
		if (mCaretPos == 0)
		{
			...
			mCaretPosition.x = rc.mPolygon->GetData(0).x + 0.8f;
			mCaretPosition.y = rc.mPolygon->GetData(0).y + 0.f;
		}
		else if (mCaretPos > 0)
		{
			Position u = mTextRenderer->DrawStringRtPosition(mInputString.c_str() + mFirstCharPos, color, &rc, &matrix, mCaretPos);
			mCaretPosition.x = u.x + rc.mPolygon->GetData(0).x + 0.8f;
			mCaretPosition.y = u.y + rc.mPolygon->GetData(0).y + 0.f;
		}
		else
		{
			//do nothing
	...
	void TextInputProcessor::CaretToUp()
	{
		...
		Shape::Rectangle rc;
		mGetInnerRectangle(rc);
		temp.x = mCaretPosition.x - rc.mPolygon->GetData(0).x - 0.8f;
		temp.y = mCaretPosition.y - rc.mPolygon->GetData(0).y;
